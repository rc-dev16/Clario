...
const AnalysisResultPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const found = contracts.find((c: AnalysisResult) => c.id === id);
      if (found) {
        setResult(found);
      } else {
    ...
    <div className="min-h-screen bg-gray-50 overflow-hidden relative">
        ...
        <div className={`transition-all duration-1000 ${isVisible ? 'animate-slide-up' : 'opacity-0'}`}>
          ...
          {result && (
            <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden mt-8">
              <AnalysisResults result={result} setShowChat={setShowChat} />
            </div>
          )}
        </div>
      </main>
      ...
      {showChat && result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-3xl h-[90vh] flex flex-row items-stretch justify-center">
            {/* Chat Modal Content */}
            ...
            <style>{`
              ...
              .animate-fade-in-glass { animation: fade-in-glass 0.0s cubic-bezier(0.4,0,0.2,1) forwards; }
            `}</style>
          </div>
        </div>
      )}
