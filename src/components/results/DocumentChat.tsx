...
const DocumentChat: React.FC<DocumentChatProps> = ({ result }) => {
    ...
    return getSessionKeys()
      .map(k => {
        ...
        return {
          ...
          preview: lastMsg ? lastMsg.text.slice(0, 40) : 'No messages',
        };
      })
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center  z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-full flex overflow-hidden">
        {/* Sidebar */}
            ...
            {getSessions().length === 0 ? (
              ...
              <div className="space-y-2">
                {getSessions().map((s) => (
                  ...
                  >
                    <div className="truncate">{s.preview}</div>
                    <div className="text-[10px] text-gray-400 mt-1">{s.lastTime ? new Date(s.lastTime).toLocaleString() : ''}</div>
                  </button>
                ))}
              </div>
            )}
