...
import { AnalysisResults } from './results/ResultDetails';
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let limitedContracts: AnalysisResult[] = [];
    if (savedContracts) {
      const parsedContracts = JSON.parse(savedContracts);
      // Sort contracts by date (newest first)
      parsedContracts.sort((a: AnalysisResult, b: AnalysisResult) => 
        new Date(b.analysisDate).getTime() - new Date(a.analysisDate).getTime()
      );
      // Limit to 5 contracts for free plan
      if (user && user.maxContracts === 5) {
        limitedContracts = parsedContracts.slice(0, 5);
      } else {
        limitedContracts = parsedContracts;
      }
  ...
  const handleDeleteContract = async (contractId: string) => {
    if (window.confirm('Are you sure you want to delete this contract analysis?')) {
      ...
      // Do NOT decrement contract count in Firestore
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-12">
        {/* Welcome Section */}
            ...
            <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden">
              <div className="bg-gray-50 px-8 py-6 border-b border-gray-200 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-gray-700 transition-colors duration-200 hover:rotate-90 transform"
                >
                  <span className="text-2xl font-bold">×</span>
                </button>
              </div>
              <div className="p-8">
                <AnalysisResults result={selectedContract} setShowChat={() => {}} />
              </div>
            </div>
          </div>
        )}
